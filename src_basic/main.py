    ...
    if config.RETRY_FAILED and failed_indices and not dry_run:
        ...
        for attempt in range(1, config.MAX_RETRY + 1):
            ...
            for peserta in retry_list:
                message = engine.render(nama_peserta=peserta.nama_peserta, **peserta.extra_vars)
...
@click.option("--nama", default="BUDI SANTOSO", help="Nama peserta untuk preview")
def preview(tpl_path, nama):
    ...
    # Preview dengan dummy data untuk kolom custom
    rendered = engine.preview(
